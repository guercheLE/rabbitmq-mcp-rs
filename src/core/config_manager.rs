...
const CONFIG_DIR_NAME: &str = ".rabbitmq-mcp";
const LOCAL_CONFIG_FILE: &str = "rabbitmq-mcp.config.yml";
...
/// (Windows, where `HOME` is not reliably set), then falls back to the
/// current directory rather than panicking if neither is set.
fn home_dir() -> PathBuf {
    std::env::var_os("HOME")
...
/// (`./rabbitmq-mcp.config.yml`) -> home file
/// (`~/.rabbitmq-mcp/config.yml`) -> system file
/// (`/etc/rabbitmq-mcp/config.yml`) -> install-dir file -> built-in
...
pub fn load_config(cli_flags: Map<String, Value>) -> Result<Config, McpifyError> {
    ...
    let layers = [
        ...
        read_yaml_if_exists(&PathBuf::from("/etc/rabbitmq-mcp/config.yml")),
