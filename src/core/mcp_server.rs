...
impl McpifyServer {
    ...
    #[tool(
        description = "Semantic search for RabbitMQ Management HTTP API operations using a natural-language query."
    )]
    async fn search(
        &self,
    ...
    #[tool(
        description = "Return the schema, path, method, and documentation for a specific RabbitMQ Management HTTP API operationId."
    )]
    ...
    #[tool(
        description = "Validate arguments, invoke a live RabbitMQ Management HTTP API API operation, and validate the response."
    )]
    async fn call(
        &self,
...
mod tests {
    ...
    async fn search_and_get_return_mcp_content_envelopes() {
        ...
        let operation_id = {
            let conn = cached_store_connection("4.3.2").unwrap();
