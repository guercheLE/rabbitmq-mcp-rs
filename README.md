...
Exposes exactly 3 tools — `search`, `get`, `call` — backed by an embedded semantic database (`mcp_store.db`), so an LLM never needs the full API surface in context. The default RabbitMQ 4.3.2 store contains 137 real Management HTTP API operations: 93 `GET`, 20 `DELETE`, 12 `PUT`, and 12 `POST`.
